...
def basicResults(clfObj,trgX,trgY,tstX,tstY,params,clf_type=None,dataset=None):
    ...
    N = trgY.shape[0]    
    #curve = ms.learning_curve(cv.best_estimator_,trgX,trgY,n_jobs=1, cv=5,train_sizes=[50,100]+[int(N*x/10) for x in range(1,8)],verbose=10,scoring=scorer)
    curve = ms.learning_curve(cv.best_estimator_,trgX,trgY,n_jobs=1, cv=5,train_sizes=[int(N*x/10) for x in range(1,8)],verbose=10,scoring=scorer)
